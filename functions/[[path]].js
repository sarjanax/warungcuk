 ...
 * ║   Versi : Rajah Niat Alkemis Semesta v33.1 — "Fixed Cloudflare"         ║
 * ║   Author  : dukunseo.com                                                 ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
'use strict';
...
class QCache {
  ...
  get(key) {
    ...
    if (Date.now() - this.ts.get(key) > this.ttl) { 
      this._del(key); 
      return null; 
    }
  ...
  set(key, value) {
    if (this.data.has(key)) { 
      ...
      this._del(this.data.keys().next().value);
    }
    this.data.set(key, value); 
    this.ts.set(key, Date.now());
    return value;
  }
...
const _apiCache     = new QCache(500, 300000);
const _pingCache    = new QCache(200, 3600000);
const _robotsCache  = new QCache(50,  86400000);
const _blackholeMap = new LRUMap(1000);
const _sacrificeMap = new LRUMap(50);
const _immortalEnvCache = new LRUMap(5);
const _adsCspCache  = new LRUMap(10);
...
const _INTENT_PATTERNS = {
  transactional: [/download|unduh|beli|harga|price|buy|shop|order/i, /gratis|free|murah|discount|promo/i],
  informational: [/cara|how to|tutorial|panduan|guide|apa itu|what is|pengertian/i, /tips|trik|rahasia|secrets|review|ulasan/i],
  navigational: [/login|masuk|daftar|sign up|register|akun|account/i, /kontak|contact|about|tentang|profil|profile/i],
};
...
const _DEFAULT_CONFIG = {
  ...
  // REMOVED ExoClick hardcode - now using empty strings
  ADS_CODE_TOP_D:    '',
  ADS_CODE_TOP_M:    '',
  ADS_CODE_CNT_D:    '',
  ADS_CODE_CNT_M:    '',
...
function h(str) { 
  if (str==null) return ''; 
  return String(str).replace(_hRx, c => _hMap[c]); 
}

function mbSubstr(str, start, len) { 
  return [...(str||'')].slice(start, start+len).join(''); 
}
...
function safeThumb(item, cfg) { 
  return item?.thumbnail || cfg.DEFAULT_THUMB; 
}
...
// URL HELPERS
// ============================================================================
function urlHelper(path='/', cfg) { 
  return (cfg.WARUNG_BASE_PATH||'')+'/'+path.replace(/^\/+/,''); 
}

function absUrl(path, cfg) { 
  return 'https://'+cfg.WARUNG_DOMAIN+urlHelper(path, cfg); 
}

function contentUrl(id, title, cfg) {
  const slug=makeSlug(title||''); 
  let p=cfg.PATH_CONTENT+'/'+id;
  if (slug) p+='/'+slug; 
  return urlHelper(p, cfg);
}

function albumUrl(id, title, cfg) {
  const slug=makeSlug(title||''); 
  let p=cfg.PATH_ALBUM+'/'+id;
  if (slug) p+='/'+slug; 
  return urlHelper(p, cfg);
}
...
function homeUrl(cfg) { 
  return cfg.WARUNG_BASE_PATH||'/'; 
}
...
function detectDomainInfo(env, request) {
  if (env.WARUNG_DOMAIN && env.WARUNG_NAME) 
    return { domain: env.WARUNG_DOMAIN, name: env.WARUNG_NAME };
  
  if (request) {
    try {
      const hostname = new URL(request.url).hostname;
      return { 
        domain: env.WARUNG_DOMAIN || hostname, 
        name: env.WARUNG_NAME || subdomainToName(hostname.split('.')[0]) 
      };
    } catch { 
      if (env.DAPUR_DEBUG==='true') console.error('Domain detection failed'); 
    }
  }
...
function getConfig(env, request) {
  ...
  if (shouldCache && _cfgCacheMap.has(cacheKey)) 
    return _cfgCacheMap.get(cacheKey);
...
function isGoogleBot(ua) { 
  return ua.includes('Googlebot')||ua.includes('Google-InspectionTool'); 
}

function isBingBot(ua)   { 
  return ua.includes('bingbot')||ua.includes('BingPreview'); 
}

function isSearchBot(ua) { 
  return isGoogleBot(ua)||isBingBot(ua)||_SEARCHBOT_RX.test(ua); 
}

function isScraperBot(ua){ 
  return _SCRAPER_BOTS.some(b=>ua.includes(b)); 
}
...
async function handleHoneypot(request, env) {
  ...
  if (env.DB) {
    env.DB.prepare('INSERT OR IGNORE INTO blocked_ips VALUES (?, ?)')
...
function sacrificeRedirect(request, domain, immortal) {
  ...
  let sacrifice = null;
  for (const [k,v] of _sacrificeMap) { 
    if (v.status==='active') { 
      sacrifice=v; 
      break; 
    } 
  ...
  if (!sacrifice) {
    const toDelete = [];
    for (const [k,v] of _sacrificeMap) { 
      if (v.status==='sacrificed') toDelete.push(k); 
    }
  ...
  if (newEnergy >= immortal.SACRIFICE_ENERGY_MAX) {
    ...
    _sacrificeMap.set(`sacrifice-${newId}.${domain}`, { 
      id: newId, 
      subdomain: `sacrifice-${newId}.${domain}`, 
      energy: 0, 
      status: 'active' 
    });
  } else {
...
function dnaGenerate(domain, path, immortal) {
  ...
  const pickWord = (s, i) => {
    ...
    if ((lsiSeed % 10) < 5 && lsi[word]) { 
      const arr=lsi[word]; 
      word=arr[lsiSeed % arr.length]; 
    }
    return word;
  };
...
class DapurClient {
  ...
  async getByTag(tag,params={}) {
    tag=(tag||'').trim(); 
    if (!tag) return this._emptyResponse();
      ...
      try {
        resp = await fetch(fetchUrl, { 
          headers:{
            ...
            'User-Agent':'PujaseraClient/33.0 ('+this.domain+')',
  ...
  async _fetchAndStore(fetchUrl, ck, path, useCache=true) {
    ...
    try {
      ...
      if (!resp.ok) { 
        if (this.debug) console.error('[DapurClient] Backend error', resp.status, 'on', path); 
        return this._errorResponse('Layanan sementara tidak tersedia.', 0); 
      }
      data = await resp.json();
    } catch(err) { 
      logError('DapurClient.fetch', err); 
      return this._errorResponse('Layanan sementara tidak tersedia.'); 
    }
  ...
  _errorResponse(message, code=0) { 
    return { status:'error', code, message, data:[], meta:{} }; 
  }
...
class SiteDNA {
  ...
  _build() {
    ...
    const suffixes = [
      ...
      `Sudah dipercaya ${['ribuan','jutaan','banyak'][(this.sDesc+4)%4]} pengguna setia.`,
  ...
  static get(domain) {
    ...
    if (!dna) { 
      dna = new SiteDNA(domain); 
      _siteDNACache.set(domain, dna); 
    }
    return dna;
  }
}
...
function getNavItems(cfg) {
  if (cfg._dapurConfig?.nav_items?.length) {
    return cfg._dapurConfig.nav_items.map(item => ({ 
      label:item.label, 
      icon:item.icon, 
      url:categoryUrl(item.type,1,cfg) 
    }));
  }
...
function getDeliveryMode(request) {
  ...
  return { 
    lite:slowNet||saveData, 
    saveData, 
    mobile:cfDev==='mobile'||_MOBILE_UA_RX.test(ua), 
    lowEnd:slowNet 
  };
}

// ============================================================================
// ADS FUNCTIONS - FIXED FOR JUICYADS
// ============================================================================
...
function getAdsSlots(cfg) {
  ...
  const cD = cfg.ADS_CODE_CNT_D, cM = cfg.ADS_CODE_CNT_M;
...
function renderBanner(name, cfg, request=null, nonce='') {
  ...
  if (slot.type==='popunder') {
    ...
    // FIXED: JuicyAds popunder - proper positioning
    return `<div class="ad-slot ad-slot--footer_popunder" style="position:fixed; bottom:0; left:0; width:1px; height:1px; overflow:hidden; opacity:0; pointer-events:none;">${code}</div>`;
  }
...
function getUniqueTheme(cfg, dna) {
  ...
  const hexToRgb = (hex) => {
    const r=parseInt((hex||'#ffaa00').slice(1,3),16), 
          g=parseInt((hex||'#ffaa00').slice(3,5),16), 
          b=parseInt((hex||'#ffaa00').slice(5,7),16);
...
class SeoHelper {
  ...
  title(baseTitle, contentId=0, type='') {
    ...
    if (!baseTitle) return this.siteName;
  ...
  description(baseTitle, contentId=0, type='', views=0) {
    baseTitle=(baseTitle||'').trim(); 
    if (!baseTitle) return this.cfg.SEO_DEFAULT_DESC;
...
<meta name="twitter:image:alt" content="${h(title)}">${twitterSite}${articleMeta}${videoMeta}`;
  ...
  contentSchema(item, canonical, playerUrl=null) {
    ...
    if (item.thumbnail) { 
      base['thumbnail']={'@type':'ImageObject','url':item.thumbnail}; 
      base['image']=item.thumbnail; 
    }
    ...
    if (type==='video') {
      ...
      Object.assign(base,{
        ...
        'embedUrl':playerUrl||canonical,
        'regionsAllowed':'ID',
        'requiresSubscription':false,
        'inLanguage':this.cfg.SEO_LANG||'id',
        'potentialAction':{
          '@type':'WatchAction',
      ...
      Object.assign(base,{
        ...
        'inLanguage':this.cfg.SEO_LANG||'id',
        'numberOfItems':item.photo_count||0,
  ...
  breadcrumbSchema(items, pageId='') {
    ...
    const schema = { 
      ...
      'itemListElement':items.map((item,i)=>{
        ...
        if(item.url) el['item']='https://'+this.domain+item.url; 
        return el;
      }) 
    };
...
async function handle404(cfg, seo, request) {
  ...
  const head=renderHead({ 
    title:'404 - Halaman Tidak Ditemukan | '+cfg.WARUNG_NAME, 
    desc:'Halaman yang kamu cari tidak ditemukan di '+cfg.WARUNG_NAME+'.', 
    canonical, 
    ogImage:cfg.SEO_OG_IMAGE, 
    ogType:'website', 
    noindex:true, 
    cfg, 
    seo, 
...
async function handleView(request, cfg, client, seo, segments) {
  ...
  const popularTags=media.tags?.slice(5,12).map(t=>`<a href="${h(tagUrl(t,cfg))}" class="${dna.cls.tag}">#${h(t)}</a>`).join('')||'';
...
<aside class="view-sidebar">
  ...
  return new Response(head+nav+popunderView+main+renderFooter(cfg,request,adNonce), { 
    status:200, 
...
async function handleSearch(request, cfg, client, seo) {
  ...
  if (searchResult) {
    if (searchResult?.status === 'error') errorMsg = searchResult.message || 'Pencarian gagal.';
      ...
      pagination = searchResult?.meta?.pagination || searchResult?.meta || {}; 
...
<section class="${dna.cls.contentArea}">${contentSection}${tagsHtml}</section>
  ...
  return new Response(head+nav+popunderSearch+main+renderFooter(cfg,request,adNonce), { 
    status:200, 
    headers:htmlHeaders(cfg,'search') 
  });
}
...
<section class="${dna.cls.contentArea}">${contentSection}</section>
  ...
  return new Response(head+nav+popunderTag+main+renderFooter(cfg,request,adNonce), { 
    status:200, 
...
<section class="${dna.cls.contentArea}">${contentSection}</section>
  ...
  return new Response(head+nav+popunderCat+main+renderFooter(cfg,request,adNonce), { 
    status:200, 
    headers:htmlHeaders(cfg,'list') 
  });
}
...
async function handleStaticPage(cfg, seo, request, slug) {
  ...
  const pages={
    [cfg.PATH_ABOUT.toLowerCase()]:{ 
      title:ev('PAGE_ABOUT_TITLE','Tentang Kami'), 
      icon:'fa-info-circle', 
      desc:ev('PAGE_ABOUT_DESC','Tentang '+cfg.WARUNG_NAME), 
      content:ev('PAGE_ABOUT_CONTENT',`<h2>Tentang ${h(cfg.WARUNG_NAME)}</h2><p>${h(cfg.WARUNG_NAME)} adalah platform streaming gratis yang hadir untuk memberikan pengalaman menonton terbaik. Akses ribuan konten video dan album tanpa registrasi, kapan saja dan di mana saja.</p><p>Kami berkomitmen untuk menyediakan konten berkualitas dengan kecepatan streaming optimal.</p>`) 
    },
    [cfg.PATH_CONTACT.toLowerCase()]:{ 
      title:ev('PAGE_CONTACT_TITLE','Hubungi Kami'), 
      icon:'fa-envelope', 
      desc:ev('PAGE_CONTACT_DESC','Kontak '+cfg.WARUNG_NAME), 
      content:ev('PAGE_CONTACT_CONTENT',`<h2>Hubungi Kami</h2><p>Ada pertanyaan atau masukan? Kami siap membantu.</p><address><p><strong>Email:</strong> <a href="mailto:${h(cfg.CONTACT_EMAIL)}">${h(cfg.CONTACT_EMAIL)}</a></p><p><strong>Nama:</strong> ${h(cfg.CONTACT_EMAIL_NAME)}</p></address>`) 
    },
    [cfg.PATH_FAQ.toLowerCase()]:{ 
      title:ev('PAGE_FAQ_TITLE','FAQ'), 
      icon:'fa-question-circle', 
      desc:ev('PAGE_FAQ_DESC','Pertanyaan yang sering diajukan tentang '+cfg.WARUNG_NAME), 
      content:ev('PAGE_FAQ_CONTENT',`<h2>Pertanyaan Umum</h2><div class="faq-list"><details><summary>Apakah layanan ini gratis?</summary><p>Ya, sepenuhnya gratis tanpa registrasi.</p></details><details><summary>Cara melaporkan konten yang bermasalah?</summary><p>Kirim email ke <a href="mailto:${h(cfg.CONTACT_EMAIL)}">${h(cfg.CONTACT_EMAIL)}</a></p></details><details><summary>Apakah perlu daftar akun?</summary><p>Tidak perlu, langsung tonton tanpa mendaftar.</p></details></div>`), 
      schema:seo.faqSchema(faqData) 
    },
    [cfg.PATH_TERMS.toLowerCase()]:{ 
      title:ev('PAGE_TERMS_TITLE','Syarat & Ketentuan'), 
      icon:'fa-file-contract', 
      desc:ev('PAGE_TERMS_DESC','Syarat dan Ketentuan penggunaan '+cfg.WARUNG_NAME), 
      content:ev('PAGE_TERMS_CONTENT',`<h2>Syarat &amp; Ketentuan</h2><p>Dengan menggunakan ${h(cfg.WARUNG_NAME)}, Anda setuju:</p><ul><li>Konten hanya untuk penggunaan pribadi dan non-komersial.</li><li>Dilarang mendistribusikan ulang tanpa izin tertulis.</li><li>Pengguna bertanggung jawab atas penggunaan layanan.</li></ul><p>Terakhir diperbarui: ${new Date().toLocaleDateString('id-ID',{year:'numeric',month:'long'})}</p>`) 
    },
    [cfg.PATH_PRIVACY.toLowerCase()]:{ 
      title:ev('PAGE_PRIVACY_TITLE','Kebijakan Privasi'), 
      icon:'fa-lock', 
      desc:ev('PAGE_PRIVACY_DESC','Kebijakan Privasi '+cfg.WARUNG_NAME), 
      content:ev('PAGE_PRIVACY_CONTENT',`<h2>Kebijakan Privasi</h2><p>Kami menghargai privasi Anda:</p><ul><li>Kami mengumpulkan data anonim untuk meningkatkan layanan.</li><li>Kami tidak menjual data pribadi kepada pihak ketiga.</li><li>Cookie digunakan untuk meningkatkan pengalaman browsing.</li></ul><p>Pertanyaan: <a href="mailto:${h(cfg.CONTACT_EMAIL)}">${h(cfg.CONTACT_EMAIL)}</a></p><p>Terakhir diperbarui: ${new Date().toLocaleDateString('id-ID',{year:'numeric',month:'long'})}</p>`) 
    },
    [cfg.PATH_DMCA.toLowerCase()]:{ 
      title:ev('PAGE_DMCA_TITLE','Kebijakan DMCA'), 
      icon:'fa-copyright', 
      desc:ev('PAGE_DMCA_DESC','Kebijakan DMCA '+cfg.WARUNG_NAME), 
      content:ev('PAGE_DMCA_CONTENT',`<h2>Kebijakan DMCA</h2><p>${h(cfg.WARUNG_NAME)} menghormati hak kekayaan intelektual. Kirim laporan ke:</p><address><p><strong>Email:</strong> <a href="mailto:${h(cfg.CONTACT_EMAIL)}">${h(cfg.CONTACT_EMAIL)}</a></p></address><p>Laporan harus menyertakan: identifikasi karya, URL konten, informasi kontak Anda, dan pernyataan keakuratan informasi.</p><p>Kami merespons dalam <strong>3 hari kerja</strong>.</p>`) 
    },
  };
...
async function handleSitemap(request, cfg, client, env, honeyPrefix, cannibal=null) {
  ...
  if (isGoogle) {
    ...
    try {
      ...
      if (cfg.DAPUR_DEBUG) console.error('Sitemap items fetch failed:',err.message); 
    }
  } else {
    const baseUrl='https://'+cfg.WARUNG_DOMAIN;
...
<rss version="2.0"
  ...
  <channel>
    ...
    <generator>WPP-PujaseraKeramat/33.1</generator>
...
class IndexingHammer {
  ...
  static get(env, cfg) {
    ...
    if (!h) { 
      h = new IndexingHammer(env, cfg); 
      _hammerCache.set(cfg.WARUNG_DOMAIN, h); 
    }
    return h;
  }
...
class Alchemist {
  ...
  _scoreCandidate(word, entries, lowerText, domain, seed) {
    ...
    const freqScore = Math.min(matches, 5);
  ...
  generateInternalLinks(text, currentItemId, maxLinks, domain) {
    ...
    for (const [word, entries] of activeIndex.entries()) {
      ...
      const score = this._scoreCandidate(word, entries, lowerText, domain, seed) - (entry.external ? 1 : 0);
...
export async function onRequest(context) {
  ...
  if (!seo) { 
    seo = new SeoHelper(cfg); 
    _seoCache.set(cfg.WARUNG_DOMAIN, seo); 
  }
  ...
  const getVisitorType = () => { 
    if (!_visitorTypeCache) _visitorTypeCache = classifyVisitor(request); 
    return _visitorTypeCache; 
  };
  ...
  if (!cannibal) { 
    cannibal = new KeywordCannibalize(cfg, env); 
    _cannibalCache.set(cfg.WARUNG_DOMAIN, cannibal); 
  }
